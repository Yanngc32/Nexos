...
describe("pastaRelativa", () => {
  it("aceita absoluta ou relativa dentro do projeto e devolve relativa com /", () => {
    const p = projeto();
    expect(pastaRelativa(p, join(p, "design", "ds"))).toBe("design/ds");
    expect(pastaRelativa(p, "design-system")).toBe("design-system");
  });

  it("recusa fora do projeto e a raiz", () => {
    const p = projeto();
    expect(() => pastaRelativa(p, "../fora")).toThrow(/dentro do projeto/);
    expect(() => pastaRelativa(p, tmpdir())).toThrow(/dentro do projeto/);
    expect(() => pastaRelativa(p, ".")).toThrow(/subpasta/);
  });
});

describe("criar / ler / salvar", () => {
  it("cria o esqueleto na pasta escolhida e já deixa ativo", () => {
    const home = tempHome();
    const p = projeto();
    const est = criarDs(p, home, { nome: "Nova Grumari", pasta: "design-system" });
    expect(est.ativo).toBe("design-system");
    expect(existsSync(join(p, "design-system", "tokens.json"))).toBe(true);
    expect(existsSync(join(p, "design-system", "DESIGN.md"))).toBe(true);
  ...
  it("adota pasta que já tem tokens.json sem sobrescrever", () => {
    const home = tempHome();
    const p = projeto();
    mkdirSync(join(p, "ds"));
    writeFileSync(join(p, "ds", "tokens.json"), JSON.stringify({ cor: { $value: "#123456" } }));
    const est = criarDs(p, home, { pasta: "ds" });
    expect(JSON.parse(readFileSync(join(p, "ds", "tokens.json"), "utf8"))).toEqual({ cor: { $value: "#123456" } });
  ...
  it("card em disco que o meta.json não conhece aparece em 'outros'", () => {
    const home = tempHome();
    const p = projeto();
    criarDs(p, home, { pasta: "design-system" });
    writeFileSync(join(p, "design-system", "cards", "extra.html"), "<p>x</p>");
  ...
  it("salvar tokens com base desatualizada dá 409; com base certa grava", () => {
    const home = tempHome();
    const p = projeto();
    const ds = criarDs(p, home, { pasta: "design-system" }).ds!;
  ...
  it("vários DS: ativar e remover (arquivos ficam no disco)", () => {
    const home = tempHome();
    const p = projeto();
    criarDs(p, home, { nome: "Painel", pasta: "ds/painel" });
    const est = criarDs(p, home, { nome: "Portal", pasta: "ds/portal" });
    expect(est.sistemas.map((s) => s.id)).toEqual(["painel", "portal"]);
    expect(est.ativo).toBe("portal");
    expect(ativarDs(p, home, "painel").ds!.nome).toBe("Painel");
    const depois = removerDs(p, home, "painel");
    expect(depois.ativo).toBe("portal");
    expect(existsSync(join(p, "ds", "painel", "tokens.json"))).toBe(true);
  });
